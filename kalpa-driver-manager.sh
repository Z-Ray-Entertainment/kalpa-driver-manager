    ...
    for i in "$@"; do
        case $i in
            ...
            if [ $is_secure_boot_enabled = true ]; then
                if [ $supported_driver_series == "$DRIVER_G06_CLOSED" ]; then
...
main(){
    ...
    if [ $user_agreed_to_license = true ]; then
        case $supported_driver_series in
            "$DRIVER_G00"|"$DRIVER_G04"|"$DRIVER_G05")
